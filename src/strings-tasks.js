...
function removeLastOccurrences(str, value) {
  ...
  if (lastIndexOf < 0) return str;
  return (
    str.substring(0, lastIndexOf) + str.substring(lastIndexOf + value.length)
  );
}
...
function isPalindrome(str) {
  const clearStr = str.toLowerCase().replaceAll(/[\W\s]/g, '');
...
function findLongestWord(sentence) {
  const wordsLengths = sentence.split(' ').map((word) => word.length);
  const max = Math.max(...wordsLengths);
  return sentence.split(' ')[wordsLengths.indexOf(max)];
}
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((ch) => {
      const indexOfInput = input.indexOf(ch);
      if (indexOfInput < 0) return ch;
      return output[indexOfInput];
    })
    .join('');
}
